...
\begin{abstract}
We present a distributed algorithm for maximum likelihood estimation (MLE) with low communication cost. % called optimal weighted averaging (OWA).
...
%In the first step, each machine independently solves the MLE problem on a local subset of the data.
%Then a second round of optimization finds the optimal weighted average of the parameter vectors from step 1.
%The resulting parameter vectors are then merged together
%OWA proceeds in two rounds.
%In the first round, each machine solves the MLE problem on a local subset of the data.
%In the second round, each machine solves a modified MLE problem.
%The number of machines $m$
%We then perform a second round of optimization to find the optimal weighted average of the results.
%Our algorithm is an easy to implement improvement to the standard averaging method.
...
%A major advantage of OWA is that two tunable parameters let the user trade better statistical p
...
%Define
%\begin{equation}
%\astar = \argmin_{\w\in\W} \int_{\X\times\Y} \left(f(y,\trans\x\w) + \lambda R(\w)\right) d(\x,y)
%\end{equation}
%The result follows by substituting Theorem 1 and Lemma 1 into Lemma 2.
\end{proof}
