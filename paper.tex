...
\begin{equation}
\begin{aligned}
%Z_i^{proj} &= \{(y,\x\hat W) : (y,\x) \in Z_i^{reopt}\}
%\\
...
%Notice that this estimator requires two rounds of communication.
...
\draw[gray] plot [smooth cycle,tension=1] coordinates {(-0.5,-0.5) (-1.05,1) (-0.1,-0.1) (0.75,-0.5) (0.45,-0.7) };
\draw[gray] plot [smooth cycle,tension=1] coordinates {(-0.85,-0.65) (-1.3,1.55) (0.25,0) (2.5,-0.5) (1,-0.95)};
\draw[gray] plot [smooth cycle,tension=1] coordinates {(-1.15,-1) (-1.5,2) (1,0) (5,0) (3,-0.95)};

\draw[thick] (-2.2,2.15) -- (5.5,1);

\node[dot] (wstar) at (-0.25,-0.25) {};
\node at (0,-0.45) {$\wstar$};
...
\node at (4.0,1.6) {$\wave$};
\node[dot] (wave) at (2.9,1.40) {};
\node at (3.1,1.75) {$\wmle_1$};
\node[dot] (wave) at (4.7,1.10) {};
\node at (4.9,1.45) {$\wmle_2$};
%\node[dot] (wave) at (3.8,1.25) {};

\node[dot] at (-1.35,2.03) {};
\node at (-1.1,2.4) {$\wreopt$};

\node at (4,-1.25) {$f(y,\trans\x\w)$};
\node at (5.75,1) {$\W$};

\end{tikzpicture}
\caption{
    Our method performs a second round of optimization to find the best parameter vector in $\W$.
    Since $\wave\in\W$, our estimator $\wreopt$ is guaranteed to have higher true likelihood with high probability.
}
\label{fig:contour}
\end{figure}
...
%\label{sec:normal}
%
%We say that a random vector $X$ is \emph{sub-Gaussian} with mean $\mu$ and covariance $\Sigma$
...
%Notice that $\Waff\subseteq\W$, so $\ltwo{\wstar-\proj\W\wstar} \le \ltwo{\wstar-\proj\Waff\wstar}$.
%\begin{equation}
%\Wtave=\{\x - \wave + \wtave : \x\in\Wave\}
%\end{equation}
%We have by the triangle inequality that
%\begin{equation}
%\begin{split}
